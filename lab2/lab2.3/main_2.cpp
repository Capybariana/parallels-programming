                                    ...
                                    const std::string& schedule_type)
{
    ...
    do {
        if (schedule_type == "dynamic") {
            #pragma omp parallel for num_threads(num_threads) schedule(dynamic, matrix_size/num_threads)
            ...
            #pragma omp parallel for num_threads(num_threads) schedule(static, matrix_size/num_threads)
            for (int i = 0; i < matrix_size; i++) {
                double Av = 0.0;
                for (int j = 0; j < matrix_size; j++) {
                    Av += A[i][j] * v[j];
